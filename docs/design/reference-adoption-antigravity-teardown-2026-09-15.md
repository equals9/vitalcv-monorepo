...
| 13 | Spacing scale as fixed steps (`--space-xs…7xl`) | **FOUNDER YES/NO** — recommendation: YES, as a UX-02A task with consumers | EC-20 spacing row: DEFERRED, UX-02A owns; measured 2026-09-15: zero `--vt-space-*` declarations **and zero references** on `origin/main`. A scale with no mounted consumer is banned by the visual gate (§1b). Adopting it means migrating at least the two public islands' paddings — a visual-gate PR of its own. |
